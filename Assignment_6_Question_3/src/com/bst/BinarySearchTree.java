...
public class BinarySearchTree {
	...
	public int countNonLeafNodes() {
	    if (isEmpty() || root == null) {
	        ...
	        return countNonLeafRecursively(root);
	    }
	}

	public int countNonLeafRecursively(Node trav) {
	    if (trav == null || (trav.left == null && trav.right == null)) {
	        return 0; // Base case: null node or leaf node
	    }

	    // Non-leaf node, so count it and recursively count its children
	    int leftCount = countNonLeafRecursively(trav.left);
	    int rightCount = countNonLeafRecursively(trav.right);

	    // Return the sum of non-leaf counts in both subtrees
